        ...
        try:
            ...
            if self.connection_method == WebRTCConnectionMethod.LocalAP:
                ...
                if not self.ip and not self.serial_number:
                    raise ValueError("IP or serial number required for LocalSTA connection")
                if self.ip:
                    self.conn = Go2WebRTCConnection(WebRTCConnectionMethod.LocalSTA, ip=self.ip)
                else:
                    self.conn = Go2WebRTCConnection(WebRTCConnectionMethod.LocalSTA, serialNumber=self.serial_number)
            elif self.connection_method == WebRTCConnectionMethod.Remote:
                if not all([self.serial_number, self.username, self.password]):
                    raise ValueError("Serial number, username, and password required for Remote connection")
